...
static void print_head_hotend_type()
{
  for (uint8_t e = 0; e < EXTRUDERS; e++)
  {
    ...
    if (old_head_pcb[e]) {
      SERIAL_PROTOCOLPGM(MSG_M115_SENSOR_TYPE_THERMISTOR);
    }
    else {
      SERIAL_PROTOCOLPGM(MSG_M115_SENSOR_TYPE_PT100);
    }

    SERIAL_PROTOCOLPGM(MSG_M115_HOTEND_TYPE);
    SERIAL_PROTOCOL_F(e, 10);

    if (low_temp_hotend[e]) {
      SERIAL_PROTOCOLPGM(MSG_M115_HOTEND_TYPE_LOW_TEMP);
    }
    else {
      SERIAL_PROTOCOLPGM(MSG_M115_HOTEND_TYPE_HIGH_TEMP);
    }
  }
}
...
static void setup_head_pcb_detect()
{
  ...
  if (!low_temp_hotend[0]) {
    heater_0_maxtemp = HEATER_HIGH_TEMP_MAX;
  }
  ...
  if (!low_temp_hotend[1]) {
    heater_1_maxtemp = HEATER_HIGH_TEMP_MAX;
  }
#endif
...
static inline float bed_width() {
  return base_max_pos(X_AXIS) - base_min_pos(X_AXIS) + bed_width_correction;
}
      ...
      case 28: //G28 Home all Axis one at a time
        {
          ...
          delay(1000); //give piezo time to settle
          // Home Z
  ...
  else if (code_seen('M'))
  {
    switch ((int)code_value())
    {
      ...
      case 207: //  M207 set maxtemp values
        ...
        if (toolnum == 0) {
          SERIAL_PROTOCOLLN(heater_0_maxtemp);
        }
        else if (toolnum == 1) {
