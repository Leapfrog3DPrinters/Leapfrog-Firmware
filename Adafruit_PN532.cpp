...
void Adafruit_PN532::PrintHex(const byte * data, const uint32_t numBytes)
{
  ...
  for (szPos=0; szPos < numBytes; szPos++)
  {
    ...
    if (data[szPos] <= 0xF)
	SERIAL_PROTOCOL(F("0"));
    	SERIAL_PROTOCOL_F(data[szPos]&0xff, HEX);
...
void Adafruit_PN532::PrintHexChar(const byte * data, const uint32_t numBytes)
{
  ...
  for (szPos=0; szPos < numBytes; szPos++)
  {
    ...
    if (data[szPos] <= 0xF)
	SERIAL_PROTOCOL(F("0"));
   	SERIAL_PROTOCOL_F(data[szPos], HEX);
    if ((numBytes > 1) && (szPos != numBytes - 1))
    {
      SERIAL_PROTOCOL(F(" "));
  ...
  if (data[szPos] <= 0x1F){
      ...
      SERIAL_PROTOCOL((char)data[szPos]);
    }
  SERIAL_PROTOCOL("");
  ...
  if (pn532_packetbuffer[0] == 0 && pn532_packetbuffer[1] == 0 && pn532_packetbuffer[2] == 0xff) {
    ...
    if (pn532_packetbuffer[5]==PN532_PN532TOHOST && pn532_packetbuffer[6]==PN532_RESPONSE_INDATAEXCHANGE) {
      ...
      *responseLength = length;
      ...
      SERIAL_PROTOCOL(F("Don't know how to handle this command: "));
      SERIAL_PROTOCOL_F(pn532_packetbuffer[6],HEX);
      return false;
    }
  }
  else {
    SERIAL_PROTOCOL(F("Preamble missing"));
    return false;
  }
}
...
bool Adafruit_PN532::inListPassiveTarget() {
  ...
  if (pn532_packetbuffer[0] == 0 && pn532_packetbuffer[1] == 0 && pn532_packetbuffer[2] == 0xff) {
    ...
    if (pn532_packetbuffer[5]==PN532_PN532TOHOST && pn532_packetbuffer[6]==PN532_RESPONSE_INLISTPASSIVETARGET) {
      if (pn532_packetbuffer[7] != 1) {
                SERIAL_PROTOCOL(F("Number of tags inlisted:"));
        SERIAL_PROTOCOL(pn532_packetbuffer[7]);
        return false;
      }

      _inListedTag = pn532_packetbuffer[8];
      SERIAL_PROTOCOL(F("Tag number: "));
      SERIAL_PROTOCOL(_inListedTag);
